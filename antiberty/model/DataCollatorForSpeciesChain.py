...
class DataCollatorForSpeciesChain(DataCollatorForLanguageModeling):
    ...
    def __call__(
        ...
        if isinstance(examples[0], (dict, BatchEncoding)):
            ...
            batched_input_ids = self.tokenizer.pad(
                input_ids, return_tensors="pt", pad_to_multiple_of=self.pad_to_multiple_of
            )
        else:
    ...
    def process_examples(self, examples):
        example_species_dict = {i: self.SPECIES_TO_LABEL[e["Species"]] for i, e in enumerate(examples)}
        ...
        for i, example in enumerate(examples):
            seq = " ".join(list(example["seq"])).replace("-", self.missing_res_token)
            ...
            examples_.append(dict(input_ids=input_ids, species_label=species_label, chain_label=chain_label))
